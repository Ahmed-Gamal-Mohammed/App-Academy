...
const makeTree = (categories, parent = null) => {
  // Initialize the tree for the current parent
  const tree = {};

  // Loop through the categories and find those with the current parent
  categories.filter(category => category.parent === parent).forEach(category => {
    // Recursively build the subtree for the current category

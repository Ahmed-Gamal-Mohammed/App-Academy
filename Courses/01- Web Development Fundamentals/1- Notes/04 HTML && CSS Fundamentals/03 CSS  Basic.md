- It's a language used to ==style== a Web Pages 
- ==CSS== stands for **Cascading Style Sheets**
- It's describes how HTML elements are displayed on your screen 
- You can also manipulate your layout with CSS. This is done by using:
	- **Felxbox**
	- **Grid**
	- **Media Queries**
...
# CSS Selector
--------------------------------------------------------------------------------------------------------------
- A Selector can be any of the following basic types 
	- **Type Selector** -- matches elements by node name(e.g ==[div]==, ==[li]== ==[a]== ==[p]==)
	- **Class Selector** -- matches elements by class name `(e.g <button class = "active">)`
	 -  **ID Selector** -- matches elements by ID name `(e.g. <div id = "list-1")`
	 - **Universal selector** -- matches elements of any type (e.g `*`)
	 - **Attributes selector** matches elements based on the presence or value of a given attribute (e.g `a[title]` matches all `a` elements with a `title` attribute)
...
Given the HTML and CSS above, the uppercase style would be applied to the ABBR elements containing "html", "css", "gif" and "ted" because they are all descendants of a DIV element. The ABBR elements containing "[nasa]()" and "[ufo]()" are not descendants of a DIV element and would remain lowercase
...
---
### Pseudo-classes
A **pseudo-class** specifies a special state of the selected element(s) and does not refer to any elements or attributes contained in the DOM (hence, _pseudo_). A pseudo-class is formed by a selector followed by a colon followed by the pseudo-class name (syntax: A:B).

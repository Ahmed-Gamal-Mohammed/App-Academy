...
function advancedExponent(b, n) {
  ...
  if (n === 1) {
    return b; // base case: b^1 = b
  }

  const half = advancedExponent(b, Math.floor(n / 2)); // recurse for half of n

  if (n % 2 === 0) {
    return half * half; // if n is even, square the result
  } else {
    return b * half * half; // if n is odd, multiply by b
  }
}

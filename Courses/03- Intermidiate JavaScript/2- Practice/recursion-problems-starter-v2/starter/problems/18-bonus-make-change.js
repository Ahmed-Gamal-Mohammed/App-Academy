...
function makeBetterChange(target, coins = [25, 10, 5, 1]) {
  // Base case: if target is 0, return an empty array
  if (target === 0) return [];

  let bestSolution = null;

  // Try each coin, and for each, recurse to find the best solution for the remainder
  for (let i = 0; i < coins.length; i++) {
    ...
    if (coin <= target) {
      ...
      // If a valid solution is found, prepend the coin to the solution
      if (changeForRemainder !== null) {
        let solution = [coin, ...changeForRemainder];

        // If this solution is better (i.e., uses fewer coins), keep it

...
## Five primitives Types
1- `Bollean` - **True and false**
2- `Null` ==> represent the intentional absence of value 
3- `Undefined`==> default return value for may things in JavaScript
4- `Number`==> Like the number we usually use (12, 44)
5- `String` ==> ordered collection of characters ("Ahmed")

## One Reference Type 
1- `Object` ==> (**an array is also king of object**)


one of the main ways _Reference Types_ and _Primitive Types_ differ
- _Primitive types_ are **immutable**. Meaning they cannot change.
## Immutability
When we talk about primitive types the first thing we mentioned was _mutability_. Primitives are **immutable** meaning they can not be directly changed. Let's look at an example:
...
![[assignment-num1.png]]Next we assign `num2` to the **value** of `num1`. What effectively happens when we do this is we are _copying_ the value of `num1` and then pointing `num2` at that copy:
![[assignment-num2.png]]
Now here is where it gets really _interesting_. We cannot change the 5 our computer has in memory - because it is a **primitive** data type. Meaning if we want `num1` to equal 8 we need to **reassign** the value of the `num1` variable. When we are talking about _primitives_ reassignment breaks down into simply having your variable point somewhere else in memory:
![[assignment-num3.png]]
...
![[assignment-num4.png]]
This holds true of arrays as well. Arrays are a kind of object - though obviously different. We'll go a lot deeper into this when we start talking about classes - but for now concentrate on the fact that arrays are also a _Reference Type_.
...
```
If we change `array1` we also change `array2` because both are pointing to the same _reference_ in the computer's memory.
